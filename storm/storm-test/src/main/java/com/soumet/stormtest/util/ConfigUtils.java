...
package com.soumet.stormtest.util;
...
import com.soumet.stormtest.fail.ReliableFailHandler;
import com.soumet.stormtest.fail.UnreliableFailHandler;

/**
 * Utilities for {@link com.soumet.stormtest.KafkaSpout} regarding its configuration and reading values from the
 * storm configuration.
 *
    ...
    public static int getMaxBufSize(final Map<String, Object> stormConfig) {
        ...
        if (value != null) {
            try {
                ...
                //LOG.warn("invalid value for '{}' in storm config ({}); falling back to default ({})", CONFIG_BUFFER_MAX_MESSAGES, String.valueOf(value).trim(), String.valueOf(DEFAULT_BUFFER_MAX_MESSAGES));
            }
        }
