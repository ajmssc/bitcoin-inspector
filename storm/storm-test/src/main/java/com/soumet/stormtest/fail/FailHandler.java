...
package com.soumet.stormtest.fail;
 ...
 * {@link com.soumet.stormtest.KafkaSpout} to determine whether failed messages should be replayed. Messages not
 * to be replayed are provided to the handler to deal with, at which point the
 * {@link com.soumet.stormtest.KafkaSpout} will continue as if the message was processed correctly. This makes it
...
public interface FailHandler extends Serializable {
     ...
     * responsible for dealing with the message when the {@link com.soumet.stormtest.KafkaSpout} should not emit
     * it again.
     *
    ...
    /**
     * Called by the {@link com.soumet.stormtest.KafkaSpout} when a tuple is acknowledged by the topology.
     *
    ...
    /**
     * Called by the {@link com.soumet.stormtest.KafkaSpout} when a tuple is failed by the topology and
     * {@link #shouldReplay(com.soumet.stormtest.util.KafkaMessageId)} indicates it should *not* be replayed.
     *
     * @param id      The failed id.
    ...
    /**
     * Called by the {@link com.soumet.stormtest.KafkaSpout} when
     * {@link com.soumet.stormtest.KafkaSpout#open(java.util.Map, backtype.storm.task.TopologyContext, backtype.storm.spout.SpoutOutputCollector)}
    ...
    /**
     * Called by the {@link com.soumet.stormtest.KafkaSpout} when
     * {@link com.soumet.stormtest.KafkaSpout#activate()} is called on it.
     */
    void activate();

    /**
     * Called by the {@link com.soumet.stormtest.KafkaSpout} when
     * {@link com.soumet.stormtest.KafkaSpout#deactivate()} is called on it.
     */
    void deactivate();

    /**
     * Called by the {@link com.soumet.stormtest.KafkaSpout} when
     * {@link com.soumet.stormtest.KafkaSpout#close()} is called on it.
     */
    void close();

    /**
     * Called by {@link com.soumet.stormtest.KafkaSpout} on opening the spout to log the failure policy used.
     *
